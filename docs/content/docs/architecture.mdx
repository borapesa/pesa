...
The default adapter is in-memory (lost on restart). Swap to a persistent adapter for production. Additional adapters (SQLite, Turso/libSQL, PostgreSQL) are planned.
