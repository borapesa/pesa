...
The handler exposes three endpoints under `/pesa/`:
- `POST /pesa/order` — create a payment
- `GET /pesa/status/:orderId` — query status
- `POST /pesa/webhook` — receive provider callbacks
