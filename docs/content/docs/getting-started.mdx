...
The handler exposes three endpoints (customizable via `basePath` in config):

- `POST {basePath}/order` — create a payment
- `GET {basePath}/status/:orderId` — query status
- `POST {basePath}/webhook` — receive provider callbacks

`basePath` defaults to `/pesa` and can be set to any prefix (e.g., `/api/payments`).
