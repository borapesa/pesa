...
- **Card payments** — `createOrder` with `redirectUrl` routes to Snippe's card endpoint (Visa/Mastercard). Returns `checkoutUrl` for the payment page.
- **Hosted checkout** — without `redirectUrl`, `createOrder` routes to mobile money USSD push.
- **Balance query** — `getBalance()` returns available and total TZS balances.
- **Order listing** — `listOrders()` supports date-range filtering for reconciliation.
- **Credential validation** — `validateCredentials()` checks API key by calling the balance endpoint.
